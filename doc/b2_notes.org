...
If it's a pain to deal with the logic in Python or whatever (and it
looks like it might be...), then why not a command line tool that
depends on beeb_lib and can use the SaveTrace code. The executive
decision can be that the consuming tool doesn't promise to deal with
files not saved out by the corresponding b2 version.

This would simplify a few things. Some of the behaviour wouldn't have
to be properly tied down, because the tool and emulator code would
match. (Particularly relevant for the BBBCMicroType.) But there'd
still be the option of running repeatedly with the same input data and
different options.
