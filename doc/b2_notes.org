...
#+begin_src markdown
  ,**Note**: a UI library update means your window layout will be lost, and you'll have to recreate it. Sorry! I hope this won't happen too often.

  ,* Add an error for unrecognised HTTP API query parameters
  ,* Add automatic CMOS/EEPROM saving
  ,* Add audio low pass filter
  ...
  ,* Improve memory emulation: fix handling of write-only I/O registers, empty ROM sockets and 1 MHz I/O regions on Master
  ,* Improve Video NuLA accuracy
  ,* macOS/Linux: prefer FLAC (lossless) audio encoding when available
  ,* Windows: video audio output is now always FLAC (lossless)
  ,* Update GUI library
#+end_src
