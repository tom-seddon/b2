...
#+begin_src markdown
  ...
  ,* Improve debugging support for memory-mapped I/O: breakpoints work, and there's partial support for viewing it in the memory and disassembly views
  ,* Fix some 1770 FDC bugs: fix Watford DDB2/DDB3 double density; fix Watford DDB2 generally
  ,* Fix debugger's view of ROMs when using BBC B without ROM board
  ,* macOS: improve support for retina displays. Should look a little nicer
  ,* Windows: improve support for high-DPI displays. Should look quite a bit better if using display scaling
  ,* Linux: improve "support" for high-DPI displays: you can use the GUI scale to fix it
#+end_src
