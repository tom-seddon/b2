...
bool OSWORD0Callback::ThreadIsStillRelevant() const {
    return true;
}
...
struct BeebThread::ThreadState {
    ...
    std::shared_ptr<Message::CompletionFun> paste_completion_fun;
    uint32_t paste_flags = 0;
...
struct BeebThread::AudioThreadData {
    ...
    std::atomic<bool> low_pass_filter = true;
    std::atomic<int> low_pass_filter_cutoff_hz{8000};
...
void BeebThread::Message::CallCompletionFunSuccess(CompletionFun *completion_fun) {
    if (completion_fun) {
        CallCompletionFunSuccess(std::move(*completion_fun));
    }
}
...
bool BeebThread::Message::PrepareUnlessReplayingOrHalted(std::shared_ptr<Message> *ptr,
                                                         CompletionFun *completion_fun,
                                                         ThreadState *ts) {
    if (!PrepareUnlessReplaying(ptr, completion_fun, ts)) {
        return false;
    }
...
void BeebThread::AllKeysUpMessage::ThreadHandle(CompletionFun *completion_fun,
    ...
    for (uint8_t i = 0; i < 0x80; ++i) {
        ...
        if (up) {
            ts->beeb_thread->ThreadSetKeyState(ts, (BeebKey)i, false);
        }
    }
}
...
bool BeebThread::JoystickButtonMessage::ThreadPrepare(std::shared_ptr<Message> *ptr,
                                                      CompletionFun *completion_fun,
                                                      ThreadState *ts) {
    if (!this->PrepareUnlessReplayingOrHalted(ptr, completion_fun, ts)) {
        return false;
    }
...
class CallSharedCompletionFunOSWORD0Callback : public ::OSWORD0Callback {
  public:
    ...
    bool ThreadIsStillRelevant() const override {
        // if the completion fun got called, no need to keepu this callback around.
...
void BeebThread::HardResetMessage::HardReset(CompletionFun *completion_fun,
    ...
    if (ts->current_config.config.beeblink) {
        if (!ts->beeblink_handler) {
            std::string sender_id = strprintf("%" PRIu64, ts->beeb_thread->m_uid);
            ts->beeblink_handler = std::make_unique<BeebLinkHTTPHandler>(ts->beeb_thread,
                                                                         std::move(sender_id),
                                                                         ts->beeb_thread->m_message_list);
    ...
    if (ts->beeb_thread->m_power_on_tone) {
...
bool BeebThread::HardResetAndChangeConfigMessage::ThreadPrepare(std::shared_ptr<Message> *ptr,
                                                                CompletionFun *completion_fun,
                                                                ThreadState *ts) {
    return PrepareUnlessReplayingOrHalted(ptr, completion_fun, ts);
}
...
bool BeebThread::SetSpeedLimitedMessage::ThreadPrepare(std::shared_ptr<Message> *ptr,
    ...
    ts->beeb_thread->m_is_speed_limited.store(m_limited, std::memory_order_release);
...
bool BeebThread::SetSpeedScaleMessage::ThreadPrepare(std::shared_ptr<Message> *ptr,
    ...
    ts->beeb_thread->m_speed_scale.store(m_scale, std::memory_order_release);

    {
        AudioDeviceLock lock(ts->beeb_thread->m_sound_device_id);

        // This resets the error, but I'm really not bothered.
        ts->beeb_thread->m_audio_thread_data->remapper = Remapper(ts->beeb_thread->m_audio_thread_data->sound_freq,
...
bool BeebThread::LoadDiscMessage::ThreadPrepare(std::shared_ptr<Message> *ptr,
    ...
    if (m_disc_image->CanClone()) {
        ...
        ts->beeb_thread->ThreadSetDiscImage(ts, m_drive, m_disc_image);
        ptr->reset();
    }
...
bool BeebThread::DeleteTimelineStateMessage::ThreadPrepare(std::shared_ptr<Message> *ptr,
    ...
    ts->beeb_thread->ThreadDeleteTimelineState(ts, this->GetBeebState());
    ptr->reset();
    return true;
}
...
bool BeebThread::StartReplayMessage::ThreadPrepare(std::shared_ptr<Message> *ptr,
    ...
    size_t index;
    if (!ts->beeb_thread->ThreadFindTimelineEventListIndexByBeebState(ts,
                                                                      &index,
                                                                      m_start_state)) {
        return false;
    }
    ...
    ts->beeb_thread->ThreadNextReplayEvent(ts);
...
bool BeebThread::StopReplayMessage::ThreadPrepare(std::shared_ptr<Message> *ptr,
    ...
    ts->beeb_thread->ThreadStopReplay(ts);
...
bool BeebThread::StartRecordingMessage::ThreadPrepare(std::shared_ptr<Message> *ptr,
    ...
    ts->beeb_thread->ThreadClearRecording(ts);

    ts->timeline_mode = BeebThreadTimelineMode_Record;

    bool good_save_state = ts->beeb_thread->ThreadRecordSaveState(ts, true);
...
bool BeebThread::StopRecordingMessage::ThreadPrepare(std::shared_ptr<Message> *ptr,
    ...
    ts->beeb_thread->ThreadStopRecording(ts);
...
bool BeebThread::StartTraceMessage::ThreadPrepare(std::shared_ptr<Message> *ptr,
    ...
    CallCompletionFunSuccess(completion_fun);
...
bool BeebThread::CloneWindowMessage::ThreadPrepare(std::shared_ptr<Message> *ptr,
    ...
    init_arguments.initial_state = ts->beeb_thread->ThreadSaveState(ts);
...
void BeebThread::StartPasteMessage::ThreadHandle(CompletionFun *completion_fun,
    ...
    if (!!*completion_fun) {
        // either way, the completion call will have to wait.
        ts->paste_completion_fun = std::make_shared<CompletionFun>(std::move(*completion_fun));
        *completion_fun = nullptr;
    }
...
bool BeebThread::StopPasteMessage::ThreadPrepare(std::shared_ptr<Message> *ptr,
                                                 CompletionFun *completion_fun,
                                                 ThreadState *ts) {
    // TODO: would be useful to be able to stop a paste when halted, surely?
    return PrepareUnlessReplayingOrHalted(ptr, completion_fun, ts);
}
...
bool BeebThread::DebugSetByteMessage::ThreadPrepare(std::shared_ptr<Message> *ptr,
                                                    CompletionFun *completion_fun,
                                                    ThreadState *ts) {
    return PrepareUnlessReplaying(ptr, completion_fun, ts);
}
#endif
...
bool BeebThread::CreateTimelineVideoMessage::ThreadPrepare(std::shared_ptr<Message> *ptr,
    ...
    size_t index;
    if (!ts->beeb_thread->ThreadFindTimelineEventListIndexByBeebState(ts, &index, m_state)) {
        return false;
    }
...
BeebThread::SetMouseMessage::SetMouseMessage(bool mouse)
    : m_mouse(mouse) {
}
...
void BeebThread::SetMouseMessage::ThreadHandle(CompletionFun *completion_fun,
                                               ThreadState *ts) const {
    (void)completion_fun; //already handled.

    ts->beeb->SetMouse(m_mouse);
}
...
#define MIXALL (disc_sound_scale * unit->disc_drive_sound + MIXSN)
    ...
    bool low_pass_filter = atd->low_pass_filter.load(std::memory_order_acquire);

    float dt = 1.f / SOUND_CLOCK_HZ;
    float rc = (float)(1. / (2 * M_PI * atd->low_pass_filter_cutoff_hz.load(std::memory_order_acquire)));
    ...
    for (size_t sample_idx = 0; sample_idx < num_samples; ++sample_idx) {
        ...
        // Nasty sample with no low pass filter. TODO: it's a bit wasteful to do
        // both... the code could do with a bit of restructuring to eliminate
        // the duplication.
        float sample2 = 0.f;
        float mix = 0.f;
        ...
        dest[sample_idx] = low_pass_filter ? fs[LOW_PASS_FILTER_ORDER - 1] : sample2;
    }
...
void BeebThread::SetLowPassFilter(bool low_pass_filter) {
    m_audio_thread_data->low_pass_filter.store(low_pass_filter, std::memory_order_release);
}

//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////

void BeebThread::SetLowPassFilterCutoff(int hz) {
    if (hz >= 1) {
        m_audio_thread_data->low_pass_filter_cutoff_hz.store(hz, std::memory_order_release);
    }
}
...
void BeebThread::ThreadStartTrace(ThreadState *ts) {
    ...
    if (any_instruction_condition) {
        ts->beeb->AddHostInstructionCallback(&ThreadHandleTraceInstructionConditions, ts);
    } else {
        ts->beeb->RemoveHostInstructionCallback(&ThreadHandleTraceInstructionConditions, ts);
    }

    if (any_write_condition) {
        ts->beeb->AddHostWriteCallback(&ThreadHandleTraceWriteConditions, ts);
    } else {
        ts->beeb->RemoveHostWriteCallback(&ThreadHandleTraceWriteConditions, ts);
    }
        ...
        //if(!messages.empty())
        {
            ...
            for (auto &&m : messages) {
                ...
                if (!m.message) {
                    ...
                    Message::CallCompletionFunSuccess(std::move(m.completion_fun));
                } else {
            ...
            if (m_is_pasting) {
                if (!ts.beeb->IsPasting()) {
                    ...
                    ThreadCallSharedCompletionFunSuccess(&ts, std::move(ts.paste_completion_fun));
                }
            }
            ...
            for (size_t i = 0; i < ts.completion_timeouts.size(); ++i) {
                CompletionTimeout *timeout = &ts.completion_timeouts[i];

                if (ts.num_executed_cycles->n < timeout->absolute_cycles.n) {
                    // Future timeouts are in the future, so don't keep looking.
                    //
                    // If the list is currently out of order, no problem - it'll get dealt with soon enough. The timeouts are approximate.
                    break;
                }
...
template <class T>
static void RemoveCallback(std::vector<std::shared_ptr<T>> *callbacks, const std::shared_ptr<T> &callback) {
    std::replace(callbacks->begin(), callbacks->end(), callback, std::shared_ptr<T>());
}
...
template <class T>
static void RemoveNulls(std::vector<T> *callbacks) {
...
void BeebThread::ThreadUpdateCallbacks(ThreadState *ts) {
    ...
    // it's ok to call if update_callbacks is false on entry. It always ends up false on exit.
    ts->update_callbacks = false;
}
...
void BeebThread::ThreadCallSharedCompletionFunSuccess(ThreadState *ts, std::shared_ptr<Message::CompletionFun> &&completion_fun) {
    if (!!completion_fun) {
        Message::CallCompletionFunSuccess(std::move(*completion_fun));
