...
Timeline::Timeline(std::unique_ptr<BeebLoadedConfig> initial_config,
                   uint64_t begin_2MHz_cycles):
m_initial_config(std::move(initial_config))
{
    this->Init2MHzCycles(begin_2MHz_cycles);
}
...
{
    this->Init2MHzCycles(m_initial_state->GetEmulated2MHzCycles());
}
...
std::unique_ptr<BBCMicro> Timeline::GetInitialBeebState() const {
    if(!!m_initial_config) {
        return m_initial_config->CreateBBCMicro(m_begin_2MHz_cycles);
    } else {
...
void Timeline::AddEvent(BeebEvent &&event) {
    this->SetEnd2MHzCycles(event.time_2MHz_cycles);
...
void Timeline::SetEnd2MHzCycles(uint64_t end_2MHz_cycles) {
    ASSERT(end_2MHz_cycles>=m_end_2MHz_cycles);
...
void Timeline::Init2MHzCycles(uint64_t num_2MHz_cycles) {
    m_begin_2MHz_cycles=num_2MHz_cycles;
    m_end_2MHz_cycles=num_2MHz_cycles;
}
