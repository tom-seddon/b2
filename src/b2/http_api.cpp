...
void from_json(const nlohmann::json &j, BBCBinaryData &s) {
    if (!j.is_string()) {
        throw nlohmann::json::type_error::create(302, strprintf("invalid BBCBinaryData value: must be a string"), nullptr);
    }
    ...
    if (!Base64Decode(&s.bytes, base64_str, nullptr)) {
        throw nlohmann::json::type_error::create(302, strprintf("invalid BBCBinaryData value: invalid base64 data"), nullptr);
    }
}
