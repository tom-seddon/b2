...
public:
    ...
    void SetBeebWindow(SDLBeebWindow *beeb_window);
protected:
    // only non-null for the duration of the DoImGui2 call.
    ...
    void DoBytePopupGui(const BBCMicro::BigPage *bp,M6502Word addr);

    // Address info, checkboxes for breakpoint flags, etc.
    void DoByteDebugGui(const BBCMicro::BigPage *bp,M6502Word addr);
...
public:
    ...
    virtual void RevealByte(const BBCMicro::BigPage *bp,M6502Word addr)=0;
protected:
private:
};
...
void DebugUI::SetBeebWindow(SDLBeebWindow *beeb_window) {
                       ...
                       bool mos)
{
    ...
    if(addr_flags) {
        if(const uint8_t *bp_addr_flags=m_beeb->DebugGetAddressDebugFlagsForMemBigPage(addr.p.p)) {
            *addr_flags=bp_addr_flags[addr.p.o];
        } else {
            *addr_flags=0;
        }
    }
...
void DebugUI::DoDebugPageOverrideImGui() {
    ...
    uint32_t dpo_mask=m_beeb->GetType()->dpo_mask;
    uint32_t dpo_current=m_beeb->DebugGetCurrentPageOverride();
...
void DebugUI::DoBytePopupGui(const BBCMicro::BigPage *bp,M6502Word addr) {
...
void DebugUI::DoByteDebugGui(const BBCMicro::BigPage *bp,M6502Word addr) {
    ...
    if(!bp) {
        ...
        if(const uint8_t *bp_addr_flags=m_beeb->DebugGetAddressDebugFlagsForMemBigPage(addr.p.p)) {
            ...
            uint8_t addr_flags=bp_addr_flags[addr.p.o];
            if(this->DoDebugByteFlagsGui(addr_str,&addr_flags)) {
                m_beeb->DebugSetAddressDebugFlags(addr,addr_flags);
            }
        ...
        char byte_str[10];
        snprintf(byte_str,sizeof byte_str,"%c%c$%04x",bp->metadata->code,ADDRESS_PREFIX_SEPARATOR,addr.w);

        ImGui::Text("Byte: %s (%s)",
                    byte_str,
                    bp->metadata->description.c_str());

        if(bp->debug) {
            uint8_t byte_flags=bp->debug[addr.p.o];
            if(this->DoDebugByteFlagsGui(byte_str,&byte_flags)) {
                m_beeb->DebugSetByteDebugFlags(bp->metadata->index,(uint16_t)addr.p.o,byte_flags);
            }
        }

        if(RevealTargetUI *reveal_target_ui=this->DoRevealGui("Reveal byte...")) {
            reveal_target_ui->RevealByte(bp,addr);
        }

        ImGui::Separator();

        if(bp->r) {
            uint8_t value=bp->r[addr.p.o];
...
static std::unique_ptr<DerivedType> CreateDebugUI(SDLBeebWindow *beeb_window) {
...
protected:
    void DoImGui2() override {
        ...
        const char *mnemonic=s->config->disassembly_info[opcode].mnemonic;
        const char *mode_name=M6502AddrMode_GetName(s->config->disassembly_info[opcode].mode);
        ...
        GetThousandsString(cycles_str,*m_beeb->GetNum2MHzCycles());
        ...
        if(m_beeb->DebugIsHalted()) {
            if(const char *halt_reason=m_beeb->DebugGetHaltReason()) {
        ...
        ImGui::Text("Address = $%04x; Data = $%02x %03d %s",s->abus.w,s->dbus,s->dbus,BINARY_BYTE_STRINGS[s->dbus]);
        ImGui::Text("Access = %s",M6502ReadType_GetName(s->read));
    }
private:
...
public:
    ...
    void RevealByte(const BBCMicro::BigPage *dbp,M6502Word addr) override {
        this->ApplyOverridesForBigPage(dbp);
        this->RevealAddress(addr);
    }
...
protected:
    void DoImGui2() override {
        ...
        this->ByteRegUI("A",s->a);
        ImGui::SameLine();
        this->ByteRegUI("X",s->x);
        ImGui::SameLine();
        this->ByteRegUI("Y",s->y);
        ImGui::SameLine();
        {
            M6502P p=M6502_GetP(s);
            char pstr[9];
        ...
        while(ImGui::GetCursorPosY()<=maxY) {
            ...
            const BBCMicro::BigPage *line_bp=this->GetBigPageForAddress(line_addr,false);
            ImGui::Text("%c%c$%04x",line_bp->metadata->code,ADDRESS_PREFIX_SEPARATOR,line_addr.w);
            this->DoBytePopupGui(line_bp,line_addr);
            ...
            if(di->num_bytes>=2) {
                ...
                const BBCMicro::BigPage *operand_l_bp=this->GetBigPageForAddress(operand_l_addr,false);
                this->DoBytePopupGui(operand_l_bp,operand_l_addr);
            ...
            if(di->num_bytes>=3) {
                ...
                const BBCMicro::BigPage *operand_h_dbp=this->GetBigPageForAddress(operand_h_addr,false);
        ...
        if(ImGui::IsWindowHovered()) {
            ...
            if(wheel<0) {
                this->Down(s->config,-wheel);
            } else if(wheel>0) {
                this->Up(s->config,wheel);
            }
        }
    }
private:
    ...
    void AddWord(const char *prefix,uint16_t w,bool mos,const char *suffix) {
        const BBCMicro::BigPage *bp=this->GetBigPageForAddress({w},mos);
        ...
        this->DoClickableAddress(prefix,label,suffix,bp,{w});
    }

    void AddByte(const char *prefix,uint8_t value,bool mos,const char *suffix) {
        const BBCMicro::BigPage *bp=this->GetBigPageForAddress({value},mos);
        ...
        this->DoClickableAddress(prefix,label,suffix,bp,{value});
    }

    void DoClickableAddress(const char *prefix,
                            const char *label,
                            const char *suffix,
                            const BBCMicro::BigPage *bp,
                            M6502Word addr)
    {
        if(prefix[0]!=0) {
            ImGui::SameLine(0.f,0.f);
...
std::unique_ptr<SettingsUI> CreateDisassemblyDebugWindow(SDLBeebWindow *beeb_window,
...
protected:
    void DoImGui2() override {
        ...
        m_beeb->DebugGetDebugFlags(&m_addr_debug_flags,&m_big_page_debug_flags);

        uint64_t new_breakpoints_change_counter=m_beeb->DebugGetBreakpointsChangeCounter();
        if(new_breakpoints_change_counter!=m_breakpoints_change_counter) {
            this->Update();
            m_breakpoints_change_counter=new_breakpoints_change_counter;
        }

#if BUILD_TYPE_Debug
        if(ImGui::CollapsingHeader("Debug Stuff",0)) {
            ...
            if(ImGui::Button("Populate")) {
                for(size_t i=0;i<100;++i) {
                    ...
                    m_beeb->DebugSetAddressDebugFlags(addr,(uint8_t)BBCMicroByteDebugFlag_BreakExecute);
                }
            }
        }
#endif
        ...
        float line_height=ImGui::GetTextLineHeight();
        {
            ...
            for(int i=clipper.DisplayStart;i<clipper.DisplayEnd;++i) {
                ...
                if(bp->big_page==0xff) {
                    ...
                    uint8_t flags;
                    if(this->Row(&flags,bp,"$%04x",bp->offset)) {
                        M6502Word addr={bp->offset};
                        m_beeb->DebugSetAddressDebugFlags(addr,flags);
                    }
                } else {
                    // Byte breakpoint
                    ...
                    uint8_t flags;
                    if(this->Row(&flags,bp,
                                 "%c%c$%04x",
                                 metadata->code,
                                 ADDRESS_PREFIX_SEPARATOR,
                                 metadata->addr+bp->offset))
                    {
                        m_beeb->DebugSetByteDebugFlags(bp->big_page,bp->offset,flags);
                    }
                }
            }
...
private:
    ...
    const uint8_t *m_addr_debug_flags=nullptr;
    const uint8_t *m_big_page_debug_flags=nullptr;//[BBCMicro::NUM_BIG_PAGES][BBCMicro::BIG_PAGE_SIZE_BYTES]={};
    ...
    bool PRINTF_LIKE(3,4) Row(uint8_t *flags,Breakpoint *bp,const char *fmt,...) {
        ...
        if(bp->big_page==0xff) {
            *flags=m_addr_debug_flags[bp->offset];
            ...
            ASSERT(bp->big_page<BBCMicro::NUM_BIG_PAGES);
            ASSERT(bp->offset<BBCMicro::BIG_PAGE_SIZE_BYTES);
            *flags=m_big_page_debug_flags[bp->big_page*BBCMicro::BIG_PAGE_SIZE_BYTES+bp->offset];
    ...
    void Update() {
        ...
        for(uint8_t i=0;i<BBCMicro::NUM_BIG_PAGES;++i) {
            const uint8_t *big_page_debug_flags=&m_big_page_debug_flags[i*BBCMicro::BIG_PAGE_SIZE_BYTES];
