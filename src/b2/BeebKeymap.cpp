...
static std::vector<std::unique_ptr<BeebKeymap>> g_beeb_keymaps;
...
static std::string GetUniqueBeebKeymapName(std::string name,BeebKeymap *ignore) {
    return GetUniqueName(std::move(name),[](const std::string &name)->const void * {
        return FindBeebKeymapByName(name);
    },ignore);
}
...
std::unique_ptr<BeebKeymap> RemoveBeebKeymapByIndex(size_t index) {
    ASSERT(index<g_beeb_keymaps.size());

    std::unique_ptr<BeebKeymap> removed_keymap=std::move(g_beeb_keymaps[index]);

    g_beeb_keymaps.erase(g_beeb_keymaps.begin()+(ptrdiff_t)index);

    return removed_keymap;
}

//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////

BeebKeymap *AddBeebKeymap(BeebKeymap new_keymap) {
    new_keymap.SetName(GetUniqueBeebKeymapName(new_keymap.GetName(),nullptr));
    g_beeb_keymaps.push_back(std::make_unique<BeebKeymap>(std::move(new_keymap)));

    BeebKeymap *keymap=g_beeb_keymaps.back().get();
    return keymap;
}
...
void BeebKeymapDidChange(size_t index) {
    ...
    BeebKeymap *keymap=g_beeb_keymaps[index].get();
...
BeebKeymap *GetBeebKeymapByIndex(size_t index) {
    ASSERT(index<g_beeb_keymaps.size());
    return g_beeb_keymaps[index].get();
}
...
BeebKeymap *GetDefaultBeebKeymap() {
    if(g_beeb_keymaps.empty()) {
        return nullptr;
    } else {
        return g_beeb_keymaps[0].get();
    }
}
