...
TVOutput::TVOutput() {
    // These vectors are never resized after creation. The data pointers
    // for a given TVOutput are constant.
    m_texture_pixels.resize(TV_TEXTURE_WIDTH*TV_TEXTURE_HEIGHT);
#if VIDEO_TRACK_METADATA
    m_texture_units.resize(m_texture_pixels.size());
#endif
//    m_texture_composited.resize(m_texture_pixels.size());
}
...
// TODO: const VideoDataUnit &...
void TVOutput::Update(const VideoDataUnit *unit) {
    switch(m_state) {
        default:
            ASSERT(0);
            break;
...

    {
        ...
        for(int y=0;y<TV_TEXTURE_HEIGHT;++y) {
            for(int x=0;x<TV_TEXTURE_WIDTH;++x) {
                if((x^y)&1) {
                    m_texture_pixels[i]=palette[1];
                } else {
                    m_texture_pixels[i]=palette[3];
                }
                ++i;
            }
        }
...
void TVOutput::CopyTexturePixels(std::vector<uint32_t> *pixels) const {
    ...
    if(m_settings.show_usec_markers||m_settings.show_half_usec_markers) {
        for(size_t x=0;x<TV_TEXTURE_WIDTH;x+=8) {
            char *dest=(char*)((uint32_t *)pixels_data+x);
            ...
            if(m_settings.show_usec_markers&&(x&15)==0) {
                for(size_t y=0;y<TV_TEXTURE_HEIGHT;++y) {
                    *(uint32_t *)dest=*(const uint32_t *)src^m_usec_marker_xor;
                    ...
                    dest+=TV_TEXTURE_WIDTH*4;
                    src+=TV_TEXTURE_WIDTH*4;;
                }
    ...
    this->AddMetadataMarkers(pixels_data,
       ...
       m_settings.show_6845_row_markers)
    {
        ...
        for(size_t x=0;x<TV_TEXTURE_WIDTH;x+=16) {
            ...
            char *dest_bytes=(char *)((uint32_t *)pixels_data+x+3);
    ...
    if(m_settings.show_beam_position) {
        if(m_x<TV_TEXTURE_WIDTH&&m_y<TV_TEXTURE_HEIGHT) {
            ...
            //} else {
            auto dest=(uint32_t *)pixels_data+m_y*TV_TEXTURE_WIDTH;
            auto src=&m_texture_pixels[m_y*TV_TEXTURE_WIDTH];
                                  ...
                                  uint32_t xor_value) const
{
    ...
    for(size_t y=0;y<TV_TEXTURE_HEIGHT;y+=2) {
        auto dest=(uint32_t *)dest_pixels+y*TV_TEXTURE_WIDTH;
