...
#define BEGIN_LOG_TRACE(OBJ,N) if(Trace *t=(OBJ)->m_trace) { Log log_value("",(OBJ)->m_trace->GetLogPrinter((N)),1),*const log=&log_value; {
#define END_LOG_TRACE() } t->FinishLog(log); } else BEGIN_MACRO {} END_MACRO

#define TRACE_STATE(OBJ,PREFIX) BEGIN_LOG_TRACE(OBJ,1000) { log->f("1770 - %s",(PREFIX)); (OBJ)->Print1770Registers(log); } END_LOG_TRACE()
