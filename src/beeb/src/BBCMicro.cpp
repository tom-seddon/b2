...
static const BBCMicro::WriteMMIOFn g_R6522_write_fns[16] = {
        &R6522::Write0, &R6522::Write1, &R6522::Write2, &R6522::Write3, &R6522::Write4, &R6522::Write5, &R6522::Write6,
        &R6522::Write7,
        &R6522::Write8, &R6522::Write9, &R6522::WriteA, &R6522::WriteB, &R6522::WriteC, &R6522::WriteD, &R6522::WriteE,
        &R6522::WriteF,
};

static const BBCMicro::ReadMMIOFn g_R6522_read_fns[16] = {
        &R6522::Read0, &R6522::Read1, &R6522::Read2, &R6522::Read3, &R6522::Read4, &R6522::Read5, &R6522::Read6,
        &R6522::Read7,
        &R6522::Read8, &R6522::Read9, &R6522::ReadA, &R6522::ReadB, &R6522::ReadC, &R6522::ReadD, &R6522::ReadE,
        &R6522::ReadF,
};

static const BBCMicro::WriteMMIOFn g_WD1770_write_fns[] = {&WD1770::Write0, &WD1770::Write1, &WD1770::Write2,
                                                           &WD1770::Write3,};
static const BBCMicro::ReadMMIOFn g_WD1770_read_fns[] = {&WD1770::Read0, &WD1770::Read1, &WD1770::Read2,
                                                         &WD1770::Read3,};
...
void BBCMicro::UpdateMaster128ROMSELPages(BBCMicro *m) {
    if (m->m_state.romsel.m128_bits.ram) {
        ...
        m->SetROMPages(m->m_state.romsel.m128_bits.pm, 0x80 + NUM_VDU_RAM_PAGES, NUM_VDU_RAM_PAGES,
                       0x40 - NUM_VDU_RAM_PAGES);
    } else {
...
void BBCMicro::Write1770ControlRegister(void *m_, M6502Word a, uint8_t value) {
    ...
    if (m->m_trace) {
        m->m_trace->AllocStringf("1770 - Control Register: Reset=%d; DDEN=%d; drive=%d, side=%d\n",
                                 m->m_state.disc_control.reset, m->m_state.disc_control.dden,
                                 m->m_state.disc_control.drive, m->m_state.disc_control.side);
    }
#endif

    LOGF(1770, "Write control register: 0x%02X: Reset=%d; DDEN=%d; drive=%d, side=%d\n", value,
         m->m_state.disc_control.reset, m->m_state.disc_control.dden, m->m_state.disc_control.drive,
         m->m_state.disc_control.side);
...
uint8_t BBCMicro::ReadAsyncCallThunk(void *m_, M6502Word a) {
    ...
    LOGF(OUTPUT, "%s: type=%u a=$%04x v=$%02x cycles=%"
            PRIu64
            "\n", __func__, m->m_state.cpu.read, a.w, m->m_state.async_call_thunk_buf[offset],
         m->m_state.num_2MHz_cycles);
...
void BBCMicro::HandleInterruptBreakpoints() {
    if (M6502_IsProbablyIRQ(&m_state.cpu)) {
        if ((m_state.system_via.ifr.value & m_state.system_via.ier.value &
             m_debug->hw.system_via_irq_breakpoints.value) ||
...
void BBCMicro::HandleCPUDataBusWithHacks(BBCMicro *m) {
    ...
    if (m->m_state.async_call_address.w != INVALID_ASYNC_CALL_ADDRESS) {
        if (m->m_state.cpu.read == M6502ReadType_Interrupt && M6502_IsProbablyIRQ(&m->m_state.cpu)) {
            TRACEF(m->m_trace,
                   "Enqueuing async call: address=$%04x, A=%03u ($%02x) X=%03u ($%02x) Y=%03u ($%02X) C=%s\n",
        ...
        if (m->m_state.cpu.read >= M6502ReadType_Opcode) {
            switch (m->m_debug->step_type) {
                ...
                case BBCMicroStepType_StepIntoIRQHandler: {
                    ASSERT(m->m_state.cpu.read == M6502ReadType_Opcode ||
                           m->m_state.cpu.read == M6502ReadType_Interrupt);
                    if (m->m_state.cpu.read == M6502ReadType_Opcode) {
                        m->SetDebugStepType(BBCMicroStepType_None);
                        m->DebugHalt("IRQ/NMI");
                    }
...
bool BBCMicro::Update(VideoDataUnit *video_unit, SoundDataUnit *sound_unit) {
    ...
    if (m_state.video_ula.control.bits.fast_6845 | odd_cycle) {
        ...
        if (output.hsync) {
            ...
            ASSERTF(addr < 32768,
                    "output: hsync=%u vsync=%u display=%u address=0x%x raster=%u; addr=0x%x; latch screen_base=%u\n",
    ...
    if (odd_cycle) {
        ...
        if (m_state.addressable_latch.bits.not_sound_write == 0 &&
            m_state.old_addressable_latch.bits.not_sound_write == 1) {
...
void BBCMicro::AddInstructionFn(InstructionFn fn, void *context) {
    ASSERT(std::find(m_instruction_fns.begin(), m_instruction_fns.end(), std::make_pair(fn, context)) ==
           m_instruction_fns.end());
...
void BBCMicro::DebugCopyMemory(void *bytes_dest_, DebugState::ByteDebugFlags *debug_dest, M6502Word addr_,
                               uint16_t num_bytes) const {
...
//                                                             MOS/FSRAM-+|
//                                                             MOS/IO-----+
void BBCMicro::DebugGetBytePointers(uint8_t **write_ptr, const uint8_t **read_ptr, uint16_t *debug_page_ptr,
                                    uint32_t full_addr) {
...
void BBCMicro::SetExtMemory(uint32_t addr, uint8_t value) {
    ExtMem::WriteMemory(&m_state.extMem, addr, value);
}
...
void BBCMicro::DebugSetAsyncCall(uint16_t address, uint8_t a, uint8_t x, uint8_t y, bool c, DebugAsyncCallFn fn,
                                 void *context) {
...
void BBCMicro::InitStuff() {
    ...
    this->SetMMIOFns(0xfc00, nullptr, &ExtMem::WriteAddressL, &m_state.extMem);
    this->SetMMIOFns(0xfc01, nullptr, &ExtMem::WriteAddressH, &m_state.extMem);
    this->SetMMIOFns(0xfc02, &ExtMem::ReadAddressL, nullptr, &m_state.extMem);
    this->SetMMIOFns(0xfc03, &ExtMem::ReadAddressH, nullptr, &m_state.extMem);

    for (uint16_t i = 0xfd00; i<= 0xfdff; ++i) {
        this->SetMMIOFns(i, &ExtMem::ReadData, &ExtMem::WriteData, &m_state.extMem);
    }

    // I/O: VIAs
    for (uint16_t i = 0; i < 32; ++i) {
        this->SetMMIOFns((uint16_t) (0xfe40 + i), g_R6522_read_fns[i & 15], g_R6522_write_fns[i & 15], &m_state.system_via);
        this->SetMMIOCycleStretch((uint16_t) (0xfe40 + i), 1);

        this->SetMMIOFns((uint16_t) (0xfe60 + i), g_R6522_read_fns[i & 15], g_R6522_write_fns[i & 15], &m_state.user_via);
        this->SetMMIOCycleStretch((uint16_t) (0xfe60 + i), 1);
    }
    ...
    if (m_disc_interface) {
        ...
        for (int i = 0; i < 4; ++i) {
            this->SetMMIOFns((uint16_t) (m_disc_interface->fdc_addr + i), g_WD1770_read_fns[i], g_WD1770_write_fns[i],
                             &m_state.fdc);

            if (m_disc_interface->flags & DiscInterfaceFlag_CycleStretch) {
                this->SetMMIOCycleStretch((uint16_t) (m_disc_interface->fdc_addr + i), true);
            }
        }
    ...
    for (uint16_t a = 0xfe00; a < 0xfe20; ++a) {
        this->SetMMIOCycleStretch(a, true);
    }
...
#if BBCMICRO_DEBUGGER
            m_shadow_pages->debug_page_index[page] = static_cast<uint16_t>(DEBUG_SHADOW_RAM_PAGE + page - 0x30);
#endif
        }
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        ASSERT(m_mmio_fn_contexts[i] == m_hw_mmio_fn_contexts[i].data() ||
...
bool BBCMicro::GetByte(uint8_t *value, uint8_t track, uint8_t sector, size_t offset) {
    if (DiscDrive *dd = this->GetDiscDrive()) {
        ...
        if (dd->track == track) {
            if (m_disc_images[m_state.disc_control.drive]) {
                if (m_disc_images[m_state.disc_control.drive]->Read(value, m_state.disc_control.side, track, sector,
                                                                    offset)) {
                    return true;
                }
            }
...
bool BBCMicro::SetByte(uint8_t track, uint8_t sector, size_t offset, uint8_t value) {
    if (DiscDrive *dd = this->GetDiscDrive()) {
        ...
        if (dd->track == track) {
            if (m_disc_images[m_state.disc_control.drive]) {
                if (m_disc_images[m_state.disc_control.drive]->Write(m_state.disc_control.side, track, sector, offset,
                                                                     value)) {
                    return true;
                }
            }
...
bool BBCMicro::GetSectorDetails(uint8_t *side, size_t *size, uint8_t track, uint8_t sector, bool double_density) {
    if (DiscDrive *dd = this->GetDiscDrive()) {
        ...
        if (dd->track == track) {
            if (m_disc_images[m_state.disc_control.drive]) {
                if (m_disc_images[m_state.disc_control.drive]->GetDiscSectorSize(size, m_state.disc_control.side, track,
                                                                                 sector, double_density)) {
