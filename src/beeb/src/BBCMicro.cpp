...
void BBCMicro::SetCRTCCursorOverrideMode(CRTCCursorOverrideMode override_mode) {
    m_state.crtc.SetCursorOverrideMode(override_mode);
}
...
void BBCMicro::StartPaste(std::vector<uint8_t> text) {
    this->StopPaste();

    if (!text.empty()) {
        m_state.hack_flags |= BBCMicroHackFlag_Paste;
        m_state.paste_state = BBCMicroPasteState_DelayBeforeStartKey;
        m_state.paste_text = std::make_shared<std::vector<uint8_t>>(std::move(text));
