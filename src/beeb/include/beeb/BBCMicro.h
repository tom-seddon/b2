...
class BBCMicro : private WD1770Handler {
  public:
    ...
    static constexpr uint32_t LED_FLAGS_FLOPPY_DISKS = ((1 << NUM_DRIVES) - 1) << BBCMicroLEDFlag_FloppyDisksShift;
    static constexpr uint32_t LED_FLAGS_HARD_DISKS = ((1 << NUM_HARD_DISKS) - 1) << BBCMicroLEDFlag_HardDisksShift;
    static constexpr uint32_t LED_FLAGS_ALL_DISKS = LED_FLAGS_FLOPPY_DISKS | LED_FLAGS_HARD_DISKS;
  ...
  public:
    ...
    // if GetCloneImpediments()!=0, returns nullptr.
    const BBCMicroUniqueState *GetCloneableUniqueState() const;
...
#include <shared/poppack.h>
    ...
    * going. */
    ...
    void StartPaste(std::vector<uint8_t> text);
    void StopPaste();
  ...
  private:
    ...
    void StepOut(uint8_t step_rate_ms) override;
    void StepIn(uint8_t step_rate_ms) override;
    ...
    void StepSound(BBCMicroState::DiscDrive *dd, uint8_t step_rate_ms);
    ...
    static const void *GetDebugMMIOReadTubeContext(const BBCMicroReadOnlyState *state);
